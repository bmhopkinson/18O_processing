function ke = fit_dark(time, Obsdata, par)
global FIG_NUM;

kvar_init(1) = 10;         % /s, forward CO2 hydration rate constant catalyzed by CA, had to /1000 to make of a similar order of magnitude as other parameters for fitting procedure (lsqcurvefit)
kvar_init(2) = 1E-8;              %cm/s, CO2 permeability
kvar_init(3) = 1E-10;              %cm/s, HCO3 permeability  

%scale parameters
kvar_init = kvar_init .* par.scale;

mink(1) = kvar_init(1)./100;
maxk(1) = kvar_init(1).*1000;
...
[kfit, resnorm, residual, exitflag, output, lambda, Jac] = lsqcurvefit('dark_lsq', kvar_init, time, Obsdata, mink, maxk, opts, par);
